    ...
    useEffect(() => {
        if (specificUser) {
            if (specificUser.length) {
                const filtPurpose = purpose.filter(p => specificUser[0]._id == p.userID)
                    ...
                    <Row>
                        <Col md={4}>
                            {specificUser[0].userImage ? (
                                <Image src={specificUser[0].userImage} id="result-image" thumbnail style={{ width: '228px', height: '190px' }}/>
                            ): (
                                <Image src={uImage} id="result-image" thumbnail style={{ width: '228px', height: '190px' }}/>
                            )}
                        </Col>
                        <Col md={1}></Col>
                        <Col md={7}>
                            <div className="result-info">
                                <h4>{(specificUser[0].firstName + ' ' + specificUser[0].middleName+' '+ specificUser[0].lastName).toUpperCase()}</h4>
                                <br/>
